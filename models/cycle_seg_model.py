...
class CycleSEGModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc,
                                        opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout, self.gpu_ids,uncertainty=opt.uncertainty,leaky_relu = opt.leaky_relu)
        self.netG_B = networks.define_G(opt.output_nc, opt.input_nc,
                                        opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout, self.gpu_ids,uncertainty=opt.uncertainty,leaky_relu = opt.leaky_relu)

        self.netG_seg = networks.define_G(opt.input_nc_seg, opt.output_nc_seg,
                                        opt.ngf, opt.which_model_netSeg, opt.norm, not opt.no_dropout, self.gpu_ids,uncertainty=opt.uncertainty,leaky_relu = opt.leaky_relu)
    ...
    def backward_G(self,epoch=0):
        ...
        if self.opt.seg_norm == 'DiceNorm':
            ...
            # print(self.loss_seg.item(),'   losss crosssss')
