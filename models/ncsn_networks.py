...
class ConvBlock_cond(nn.Module):
    def __init__(self, in_channel, out_channel, t_emb_dim, kernel_size=4, stride=1, padding=1, norm_layer=None,
                 downsample=True, use_bias=None):
        super().__init__()
...
class ResnetGenerator_ncsn(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9,
        ...
        for i in range(n_blocks):  # add ResNet blocks
            ...
            self.model_res += [
                ResnetBlock_cond(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
                                 use_bias=use_bias, temb_dim=4 * ngf, z_dim=4 * ngf)]
...
class ResnetBlock_cond(nn.Module):
    ...
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias, temb_dim, z_dim):
        ...
        self.conv_block, self.adaptive, self.conv_fin = self.build_conv_block(dim, padding_type, norm_layer,
                                                                              use_dropout, use_bias, temb_dim, z_dim)
    ...
    def forward(self, x, time_cond, z):
        ...
        for n, layer in enumerate(self.conv_block):
            ...
            if n == 0:
                # print(out.shape)
                # print(time_input[:, :, None, None].shape)
